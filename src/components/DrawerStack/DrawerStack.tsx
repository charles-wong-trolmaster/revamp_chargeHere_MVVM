...
interface DrawerStackContextType {
	...
	registerDrawer: (path: string, content: React.ReactNode, widthMultiplier?: number) => void; // Add widthMultiplier parameter
...
// Main Component
const DrawerStack: React.FC<DrawerStackProps> = ({ activeDrawer, children }) => {
	// State
	...
	const [registeredDrawers, setRegisteredDrawers] = useState<Map<string, DrawerRegistration>>(new Map());
	...
	const animationTimings = useMemo(() => {
		const duration = (styles?.drawerTransitionDuration && parseInt(styles.drawerTransitionDuration.replace('ms', ''))) || 300;
		const stagger = (styles?.drawerStaggerDelay && parseInt(styles.drawerStaggerDelay.replace('ms', ''))) || 150;
	...
	const activeRoot = useMemo(() => {
		return activeDrawer && rootDrawerIds.includes(activeDrawer) ? activeDrawer : rootDrawerIds[0];
	}, [activeDrawer, rootDrawerIds]);

	// Drawer registration
	const registerDrawer = useCallback((path: string, content: React.ReactNode, widthMultiplier?: number) => {
		const level = (path.match(/\//g) || []).length;
		const registration: DrawerRegistration = { path, content, level, widthMultiplier: widthMultiplier || 1 }; // Default to 1
	...
	const createSafeAnimation = useCallback(
		(drawerId: string, type: 'opening' | 'closing', callback: () => void, delay: number, cleanupCallback?: () => void) => {
	...
	const forceCleanupStuckStates = useCallback(() => {
		const now = Date.now();
		const maxAnimationTime = animationTimings.duration + animationTimings.stagger * 10;
	...
	const closeDrawersWithAnimation = useCallback(
		...
		[animationTimings, cleanup, createSafeAnimation, safeSetState, forceCleanupStuckStates]
	);
	...
	const findDrawerPath = useCallback(
		(drawerId: string): string | null => {
			...
			const matchingPaths = registeredPaths.filter((path) => path.endsWith('/' + drawerId));
			...
			const activeRootPaths = matchingPaths.filter((path) => path.startsWith(activeRoot + '/'));
			if (activeRootPaths.length > 0) {
				return activeRootPaths[0];
			}

			// Check visible drawer contexts
			const sortedVisibleDrawers = Array.from(visibleDrawers).sort((a, b) => b.length - a.length);
			for (const visiblePath of sortedVisibleDrawers) {
				const contextPaths = matchingPaths.filter((path) => path.startsWith(visiblePath + '/'));
	...
	const openDrawer = useCallback(
		(drawerId: string) => {
			const tryOpenDrawer = (retryCount = 0) => {
				...
				const toClose = currentVisible.filter((visiblePath) => {
					...
					for (let level = 1; level <= Math.max(visibleLevel, targetLevel); level++) {
						...
						if (level <= visibleLevel && level <= targetLevel) {
							...
							if (visibleParentAtLevel === targetParentAtLevel && visibleAtLevel !== targetAtLevel) {
								return true;
							}
						}
						...
						if (level <= targetLevel && visibleLevel > level) {
							const targetAtLevel = pathParts.slice(0, level + 1).join('/');
							if (visiblePath.startsWith(targetAtLevel.split('/').slice(0, -1).join('/') + '/') && !visiblePath.startsWith(targetAtLevel + '/')) {
								return true;
							}
						}
					}
		...
		[visibleDrawers, findDrawerPath, closeDrawersWithAnimation, openDrawersWithAnimation]
	);
	...
	const closeDrawer = useCallback(
		(drawerId: string) => {
			...
			const toClose = Array.from(visibleDrawers).filter((path) => path === targetPath || path.startsWith(targetPath + '/'));
	...
	const contextValue = useMemo(
		...
		[openDrawer, closeDrawer, openDrawerGlobal, registerDrawer, unregisterDrawer, visibleDrawers, closingDrawers]
	);
				...
				{Array.from(registeredDrawers.values())
					...
					.map((registration) => {
						...
						return isVisible || isClosing ? (
							<div
								key={path}
								className={`drawer ${isVisible ? 'visible' : ''} ${isClosing ? 'closing' : ''}`}
								data-level={level}
								data-path={path}
								style={
									{
									...
									} as React.CSSProperties & { '--drawer-width-multiplier': number }
								}
							>
								{content}
							</div>
